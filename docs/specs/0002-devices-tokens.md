...
7. Список устройств (`GET /devices`) в этой спеке отдаёт **и свои (owned), и те, к которым есть viewer_binding** — само разделение по ролям и фильтрация нужны Android-приложению с Milestone 6, но эндпоинт логично сделать сразу полным, а не переделывать в Milestone 4.
   **Правка (Milestone 4, при реализации `0005-viewer-messages-api.md`):** viewer-строка учитывается, только если `viewer_binding` **активен** — т.е. его `device_download_tokens.expires_at` ещё `NULL` либо не наступил. Изначально (при первой реализации) TTL токена проверялся только в момент создания `viewer_binding` (`POST /devices/bindings`), но не при последующих чтениях — обнаружено как несогласованность при реализации Milestone 4 и явно решено владельцем продукта: TTL должен ограничивать доступ на всё время его действия, не только в момент подключения. Правка затрагивает `storage.ListViewerDevices` (JOIN с `device_download_tokens`, фильтр по `expires_at`) и, соответственно, `GET /devices` и `GET /devices/{id}` (через общий `DeviceService.GetDevice`). Явная ревокация токена (`DELETE .../download_tokens/{token_id}`) по-прежнему действует немедленно и не дожидается TTL.
