...
- Сопоставление `systemSmsId` по `(sender, timestamp)` при бэкфилле — не строго уникально при коллизии секунды; для личного масштаба использования риск принят, не блокирует. Реализовано как окно ±2000мс (не точное совпадение — `DATE`/`DATE_SENT` в `content://sms` не совпадают миллисекунда-в-миллисекунду с локальными `sentStamp`/`receivedStamp`, carrier vs. local время) — конкретная величина окна не была зафиксирована в спеке заранее, решение принято при реализации.
...
- **Уточнено с владельцем продукта в этой сессии** (не было явно зафиксировано в исходном тексте спеки): импорт настроек **полностью заменяет** правила фильтрации на целевом устройстве (не сливает с уже существующими) — предотвращает дубли при повторном импорте одного файла.
- **Техническая поправка номера версии миграции** (не изменение требования) — спека предполагала `GatewayDatabase.version = 6`/`MIGRATION_5_6`; этот слот уже занят индексом из Milestone 18 (спека 0018 написана раньше). Реализовано как `version = 7`/`MIGRATION_6_7` — назначение и форма поля `systemSmsId` не изменились.
- **Находки `peer-review-template` (обе исправлены):** (1) замена правил фильтрации при импорте (`deleteAll()` + цикл `upsert()`) не была обёрнута в `@Transaction`, вопреки заявленной в коде атомарности — исправлено новым `FilterRuleDao.replaceAll()` (`@Transaction`). (2) сортировка кандидатов бэкфилла (`findUnmatchedForBackfill`) учитывала только близость `receivedStamp`, хотя `WHERE`-условие может сматчить строку и по `sentStamp` — исправлено на `ORDER BY MIN(...)` по обоим полям.
- **Живая проверка не покрыла целиком:** «удалять после успешной пересылки» проверено вживую только для самого механизма удаления из `content://sms` (тот же код, что и ручное удаление, — см. критерии приёмки), но не как сквозной сценарий с реальной доставкой на backend; экспорт/импорт проверен как раунд-трип в памяти (`GatewaySettingsExporterTest`), не как реальный файл через SAF на переустановленном приложении. Оба механизма (SAF-обвязка, вызов `deleteMessage` из воркера) переиспользуют уже отдельно живо проверенный код, риск расхождения оценён как низкий — не блокирует `Implemented`, по аналогии с прошлыми milestone'ами.
